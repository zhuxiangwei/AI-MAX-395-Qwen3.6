...
F16 KV cache 在 UB=512 和 UB=1024 下测试，对比 UB=256 基线。
...
| Prompt | UB=256 | UB=512 | UB=1024 |
|--------|--------|--------|--------|
| p128 | 340.2 | **351.9** | 344.4 |
| p4K | 391.3 | 474.2 | **492.7** |
| p32K | 520.1 | 634.4 | **662.1** |
| p64K | 448.1 | **542.2** | 531.9 |
...
| Prompt | UB=256 | UB=512 | UB=1024 |
|--------|--------|--------|--------|
| p128 | 0.37s | **0.36s** | 0.37s |
| p4K | 10.5s | 8.64s | **8.31s** |
| p32K | 63.0s | 51.65s | **49.49s** |
| p64K | 146.2s | **120.9s** | 123.2s |
...
| Prompt | UB=256 | UB=512 | UB=1024 |
|--------|--------|--------|--------|
| p128 | 10.23s | **10.04s** | 11.01s |
| p4K | 28.28s | **27.70s** | 27.99s |
| p32K | 76.01s | **65.17s** | 69.89s |
| p64K | 168.82s | **146.27s** | 140.54s |
| p128K | 406.57s | **356.37s** | 485.81s |
| p256K | **1040.83s** | 1200.3s | 1862.44s |

**结论：**

1. **UB=512 在 ≤128K 上下文最优**——prefill +14~22%，TTFT -12~18%，总耗时 -2~14%
2. **UB=512 Gen 速度持平**——各 prompt 大小与 UB=256 差异在 1~2 t/s 内
3. **UB=256 在 256K 上下文仍最优**——p256K prefill 快 13%，总耗时快 15%（省 ~160s）
4. **UB=1024 短 prompt prefill 最快但 p128K+ 严重劣化**——p256K prefill 跌至 132 t/s（-44%），TTFT 膨胀至 1828s（+80%）
5. **所有 UB 均无 Vulkan 崩溃**——F16 KV UB=512 和 UB=1024 均完成全部 6 个测试点（含 p256K）
...
|--------|-----------|-------------|--------------|--------------|
| p128 | 340.2 | 364.2 | 297.9 | 338.2 |
| p4K | 391.3 | 468.8 | **480.0** | 450.0 |
| p32K | 520.1 | 544.9 | **589.0** | 576.1 |
| p64K | 448.1 | 421.3 | **450.8** | 440.4 |
| p128K | **344.6** | 287.5 | 298.7 | 295.2 |
| p256K | **238.2** | 199.9 | 188.2 | 185.8 |

**Gen 速度 (t/s)：**

| Prompt | F16 ub=256 | Q8_0 ub=512 | Q8_0 ub=1024 | Q8_0 ub=2048 |
|--------|-----------|-------------|--------------|--------------|
| p128 | **54.4** | 51.6 | 53.5 | 52.5 |
| p4K | **56.6** | 51.3 | 50.2 | 52.7 |
| p32K | **46.9** | 45.3 | 46.1 | 43.9 |
| p64K | **45.5** | 43.7 | 40.7 | 41.2 |
| p128K | **36.5** | 34.0 | 33.3 | 33.3 |
| p256K | **28.4** | 24.3 | 25.4 | 25.9 |

**总执行时间 (秒)：**

| Prompt | F16 ub=256 | Q8_0 ub=512 | Q8_0 ub=1024 | Q8_0 ub=2048 |
|--------|-----------|-------------|--------------|--------------|
| p128K | **407** | 477 | **469** | 468 |
...
1. **Q8_0 KV UB=1024 是 Q8_0 配置中的最优**——p4K–p128K prefill 最快，gen 和 TTFT 接近最优，p128K 总耗时最短
2. **Q8_0 KV UB=512 是唯一确认能完成 256K 的 Q8_0 配置**——prefill 199.9 t/s，gen 24.3 t/s；但仍不如 F16 KV UB=256（prefill -16%，gen -14%，总耗时 +30%）
3. **即使最优 Q8_0 配置 (ub=1024) 在 p64K+ 仍不如 F16 ub=256**——prefill -2~13%，gen -7~9%
4. **结论：35B MoE F16 KV UB=512 适合 ≤128K，UB=256 适合 256K。** Q8_0 KV 无优势。Vulkan FA 反量化开销超过 Q8_0 压缩节省的带宽；MoE 稀疏 KV cache（3B 激活参数）意味着可节省的带宽本就有限。
...
2. **UB=1024 仅在 p256K prefill 优于 UB=256**（+69%），但在 p128K/p256K TTFT 比 UB=512 慢 38–60%
3. **Q8_0 KV 使 256K 上下文可用**——F16 KV 256K 超时；Q8_0 KV 可在 33–53 分钟内完成
4. **Q8_0 KV 大幅提升长上下文 prefill**——p128K: +132% vs F16；p64K: +37% vs F16
5. **UB≥2048 Vulkan 崩溃**——UB=2048 在 p256K 崩溃；27B Dense VRAM 余量比 35B MoE 更紧张
6. **UB=512 p256K 总执行时间 33 分钟 vs UB=256 的 53 分钟**——每次 256K 请求省 20 分钟

### 27B Dense Q6 (Q6_K_XL, 别名 `276`)

Dense 架构 Q6 量化。**F16 KV cache 在 64K+ 上下文 OOM** — Q8_0 KV cache 是长上下文必选项。**推荐配置：Q8_0 KV cache + ub=512** — 全部 6 个测试点通过，含 p256K。
...
> 配置：`-c 262144 -b 4096 -ub 512 -t 8`，F16 KV cache。27B Q6 权重比 Q8 更大，长上下文时 VRAM 余量不足。
...
2. **F16 KV 超过 32K 不可用** — p64K 超时，p128K/p256K OOM
3. **Q8_0 KV UB=512 vs 1024 差异微小** — UB=512 在 p64K+ 略快，UB=1024 在 p32K prefill 略快
4. **256K 上下文 Q8_0 KV 可用** — p256K 约 52 分钟完成 (UB=512)

### 27B Dense Q4 (Q4_K_XL, 别名 `274`)

Dense 架构 Q4 量化——最小模型，生成最快。**F16 KV cache 仅支持到 4K** — Q8_0 KV cache 是实际使用的前提。**推荐配置：Q8_0 KV cache + ub=1024** — 全部 6 个测试点通过，含 p256K。
...
> 配置：`-c 262144 -b 4096 -ub 1024 -t 8`，F16 KV cache。Q4 权重 + F16 KV 在 32K+ 超出 VRAM 容量。
...
1. **Q8_0 KV UB=1024 是唯一全范围可用配置** — 6 点全部通过，p256K 约 50 分钟
2. **F16 KV 实际不可用** — 仅 p128 和 p4K 可跑
3. **UB=2048 在 p256K Vulkan 崩溃** — 与其他模型相同的 VRAM 限制模式
4. **27B Q4 是最快的 Dense 模型** — gen ~10–25 t/s vs Q6 的 8–18 t/s vs Q8 的 7–13 t/s
...
> 35B MoE Gen 速度含 thinking tokens。Dense 模型 Gen 速度也含 thinking。所有配置使用 Q8_0 KV cache + 各模型最优 UB 值，35B MoE 除外（F16 KV，Q8_0 KV 对 MoE 无优势）。
...
| 27B Dense 最大上下文 | 256K (Q8_0 KV, ub=512) | F16 KV 256K 超时；Q8_0 KV ub=512 可用（33 分钟）；ub≥2048 崩溃 |
...
*测试环境：FEVM faex1 · AMD Ryzen AI Max+ 395 · 128 GB · llama.cpp b9210 Vulkan · 2026-05-23*
