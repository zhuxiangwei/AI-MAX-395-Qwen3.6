...
| 服务：`--models-max 2` | 允许两个模型共存常驻（如 278 + 35xb）；设为 1 可切换单模型 LRU 模式 |
...
>
> **部署模式：**
> - **双模型常驻**：models-max 2 → 278 + 35xb 共存常驻，无 LRU 淘汰，不加 --sleep-idle-seconds。所有模型 parallel = 1。
...
server {
    ...
    location /v1/ {
        limit_req zone=llm burst=5 nodelay;   # 限流
...
> 双模型模式：278 + 35xb 共存常驻，不加 `--sleep-idle-seconds`（防止重载循环导致 OOM）。`--timeout 3600` 覆盖 278 上 256K prefill 最坏情况（~3022s）并留有余量。`--cache-ram` 保持默认 8192 MiB —— 防止一模型的 prompt cache 在双模型加载时挤占另一模型的内存（见已知问题）。改为 `--models-max 1` 可切换单模型 LRU 模式（一次加载一个模型，冷切换 8–17 秒）。
...
- [ ] `~/model/router-preset.ini` 配置正确（活跃模型参数 + alias：35xb/358/278；35xq/276/274 已删除）
- [ ] 云端：`curl http://127.0.0.1:8080/v1/models` 返回模型 + aliases
- [ ] 双模型模式：首次请求后 274 和 35xb 均显示 status `loaded`
...
**受影响场景：** 主模型（27B Q8）+ 35xq 辅助模型在双模型模式下共存（`models-max 2`），且配置了 `--sleep-idle-seconds`。
...
**受影响模型：** 仅 35B-A3B APEX I-Quality（别名 `35xq`）。其他模型（358/35xb/278/276/274）不受影响。
