...
Tests 274 (27B Dense Q4) at p128/p4K/p32K/p64K/p128K/p256K.
Covers F16 KV (UB=1024) and Q8_0 KV (UB=1024/2048).

Usage (on inference machine):
    LLM_BASE_DIR=/home/zxw LLM_API_KEY=xxx python3 -u 04.bench_274.py

Filter by KV type and/or UB:
    python3 -u 04.bench_274.py --kv f16
    python3 -u 04.bench_274.py --kv q8_0 --ub 1024
"""
...
MODEL_PATH = os.path.join(_BASE_DIR, "model/Qwen3.6-27B-UD-Q4_K_XL.gguf")
...
def main():
    parser = argparse.ArgumentParser(description="04 — 27B Dense Q4 benchmark (F16 + Q8_0 KV)")
    ...
    print("=" * 64)
    print("  27B Dense Q4 — Full Benchmark")
    ...
    try:
        for kv, ub in run_plan:
            ...
            csv_file = os.path.join(_BASE_DIR, f"test/bench_274_{kv_tag}_ub{ub}.csv")
