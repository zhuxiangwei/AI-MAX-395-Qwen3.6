...
所有基准测试均在 {your_machine} (AMD Ryzen AI Max+ 395, 128 GB LPDDR5X, Radeon 8060S, llama.cpp b9625 Vulkan) 上完成。速度通过 API `timings` 测量（服务端，排除网络延迟）。Gen 速度含 thinking tokens。
...
**生成速度（按上下文长度，201 任务，cache-ram=65536）：**
...
**冷启动 Prefill 速度（按上下文长度，cache-ram=65536）：**
...
> 生产日志数据（F16 KV，UB=256，cache-ram=65536，llama.cpp b9625）。MTP 接受率：均值 83.7%，中位数 85.7%。~130K token 上下文 KV 缓存约 318 MiB（~2.4 KB/token，因 MoE 稀疏头数少）。Checkpoint 驱逐观察（143 次/225 次创建），表明 cache-ram 对单对话工作负载充足，但跨对话缓存复用受 Qwen3 SWA 架构限制。
>
...
#### F16 KV UB=256（当前配置, b9625）

**Gen 速度按响应长度分布**（378 个任务, decoded ≥ 100 tokens）：
...
**Prefill 速度按提示长度分布**（413 个完整任务, tokens ≥ 100）：
...
> † 生产负载日志数据（F16 KV, UB=256, cache-ram=49152, llama.cpp b9625, 378 gen + 413 prefill 任务）。Gen 速度含 thinking tokens。短/中上下文 gen 稳定 11–13 t/s；长上下文（5K+ decoded）8–10 t/s。Prefill 随缓存命中率变化较大；短上下文（<5K）P50 偏低因为多为 cache 命中后的增量 prefill。真实冷启动 prefill：16K+ 达 150–226 t/s（中位数 ~200 t/s），130K+ 约 45 t/s。
>
...
- 切换至单模型模式（`models-max 1`），278 默认常驻加载
...
**状态：** 部分解决——已升级至 b9625（F16 KV 安全），量化 KV + Vulkan + MTP 仍不可用；等待上游修复
...
**Workaround：** 已升级至 b9625（当前使用 F16 KV 不触发此 bug）。在上游修复 Vulkan + 量化 KV cache 交互前**不要**启用量化 KV 类型。
...
**状态：** 已缓解——prompt cache 现在在 INI 中每模型配置（278 `cache-ram = 49152`，358 `cache-ram = 65536`），不再使用服务级 `--cache-ram -1`。单模型模式 + 每模型限制防止无限增长。

**受影响场景：** 单模型模式（`models-max 1`）下 278 加载并处理长上下文请求。**已通过每模型 `cache-ram` 限制缓解。**
...
**当前缓解措施：**
- 单模型模式 + 每模型 `cache-ram` 限制（`--models-max 1`），278 默认加载，cache-ram=49152
- 每模型 `--slot-save-path` KV checkpoint 保存/恢复
- GTT 120GB + mlock=1 确保模型权重常驻物理内存
- **切勿**使用 `--cache-ram -1`（任何模式）
...
*测试环境：{your_machine} · AMD Ryzen AI Max+ 395 · 128 GB · llama.cpp b9625 Vulkan · 2026-06-03 · 更新于 2026-06-14（单模型模式，cache-ram 49152/65536，Hermes v0.16.0，超时 7200s）*
