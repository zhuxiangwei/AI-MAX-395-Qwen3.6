...
**主力模型——生成速度最快，256K 上下文稳定运行。**

| 场景 | 配置 | Gen 速度 | Prefill 速度 | MTP 命中率 |
|------|------|----------|-------------|-----------|
| 短 prompt (冷启动) | `-c 262144 -b 4096 -ub 256 -t 8` | ~72 t/s | ~114 t/s | ~100% |
| 256K 长上下文 | `-c 262144 -b 4096 -ub 256 -t 8` | 28–31 t/s | 260–285 t/s | 54–73% |

> MoE 架构每个 token 仅激活 35B 中的 3B 参数 → 内存带宽消耗极低 → 生成速度最快。

### 27B Dense（全量化等级）

**短 prompt 客户端速度（端到端，Router Mode, 2026-05-19 测量）：**

| 量化 | 别名 | Gen 速度 | MTP 命中率 | 客户端延迟 (512 tokens) |
|------|------|----------|-----------|----------------------|
| Q4_K_XL | `274` | ~21 t/s | ~80% | 30.3s |
| Q6_K_XL | `276` | ~15 t/s | ~80% | 36.8s |
| Q8_K_XL | `278` | ~11 t/s | ~68% | 54.3s |

**Dense 模型速度随模型大小近似线性变化**——瓶颈在内存带宽（理论 256 GB/s，实际 ~200 GB/s）。量化越低 → 模型越小 → 生成越快。

| 量化 | 模型大小 | Gen 速度 | 相比 Q8 |
|------|----------|----------|---------|
| Q8_K_XL | ~34 GB | 11.32 t/s | 基准 |
| Q6_K_XL | ~25 GB | 15.09 t/s | +33% |
| Q4_K_XL | ~17 GB | 21.22 t/s | +87% |

### 27B Q8 上下文边界测试（2026-05-20, b=4096 ub=512 t=8）

| ctx | prompt tokens | prefill (t/s) | gen (t/s) | MTP% | GTT peak | 状态 |
|-----|:---:|:---:|:---:|:---:|:---:|:---:|
| 32K | 26,856 | 201.0 | 11.90 | 68.4% | 38.5 GB | ✅ 稳定 |
| 64K | 53,962 | 85.7 | 11.01 | 66.8% | 40.7 GB | ✅ 稳定 |
| 96K+ | — | — | — | — | — | ⚠️ 未完成 |

> GTT 线性增长：每 32K 上下文约增 2.2 GB。Prefill 随上下文长度显著衰减（Dense 模型 KV cache 线性增长）。Gen 速度稳定 ~11 t/s（带宽瓶颈，与上下文长度无关）。

### 27B Q8 256K 全部失败（2026-05-20）
...
| ≥2048 | Vulkan DeviceLostError | 更早崩溃 |

> ⚠️ **27B Dense 256K 不可行**。Dense 全参数激活，KV cache per token 远大于 MoE，256K 超出 Vulkan 驱动容量。安全上限约 64K–96K。

### 256K 上下文关键参数：ub（35B-A3B MoE 实测）

`ub`（微批次大小）控制 GPU 单次前向传播的 VRAM 峰值。是 35B MoE 256K 稳定性的核心变量：
...
**按量化等级分配 ub 的策略：**
- Q8 → ub=256（权重最大，VRAM 余量最小，最保守）
- Q6 → ub=512（中等余量）
- Q4 → ub=1024（权重最小，VRAM 余量最大，prefill 可更快）
...
*测试环境：FEVM faex1 · AMD Ryzen AI Max+ 395 · 128 GB · llama.cpp b9210 Vulkan · 2026-05-20*
