...
**Gen 速度按响应长度分布**（283 个任务, decoded ≥ 100 tokens）：
...
**Prefill 速度按提示长度分布**（336 个完整任务, tokens ≥ 100）：
...
> † 生产负载日志数据（F16 KV, UB=256, cache-ram=32768, llama.cpp b9625, 283 gen + 336 prefill 任务）。Gen 速度含 thinking tokens。短/中上下文 gen 稳定 11–13 t/s；长上下文（5K+ decoded）8–10 t/s。Prefill 随缓存命中率变化较大；10K–30K 受 prompt cache 复用增益最明显。
>
