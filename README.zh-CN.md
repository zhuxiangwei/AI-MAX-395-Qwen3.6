...
| 按量化等级差异化 ub | 量化越高权重越大，VRAM 余量越小，需更小 ub 保证稳定性；最优 UB 因模型而异（256–1024）。当前部署：统一 UB=256（经测试 UB=512/1024 在长上下文下退化后，稳定性优先） |
| `--cache-ram 0` | 单模型轮换模式下禁用 prompt cache。此前 `--cache-ram -1`（无限制）导致 cache 无限增长但收益不成比例。双模型常驻模式使用默认 8192 MiB 限制，防止一模型挤占另一模型内存（见已知问题） |
...
| `--cache-ram` | `0`（禁用，在 service 中设置） | prompt cache 已禁用；`--cache-ram -1` 曾导致无限增长但收益不成比例；双模型常驻时须用默认 8192 MiB 防止争抢（见已知问题） |
...
|------|--------|------|------|------|------|----------|------|
| **35xb** | `Qwen3.6-35B-A3B-APEX-MTP-I-Balanced.gguf` | APEX-35B | APEX 混合精度 | **MoE** | ~24 GB | 3B | Hermes 未注册（llama-server 提供手动切换） |
...
>
> **部署模式：**
> - **单模型轮换**：models-max 1 → 一次加载一个模型，通过 LRU 按需切换（冷加载 30–60 秒），配合 `--slot-save-path` 保存/恢复 KV checkpoint。GTT 120GB + mlock=1 + cache-ram=0。不加 --sleep-idle-seconds。
>
...
> 单模型轮换模式：`--models-max 1` + `--slot-save-path` KV checkpoint 保存/恢复 + `--cache-ram 0`（prompt cache 已禁用）。模型切换延迟 30–60 秒（冷加载 + slot restore）。`--timeout 3600` 覆盖 278 上 256K prefill 最坏情况（~3022s）并留有余量。GTT 120GB + mlock=1 确保模型权重常驻物理内存。`LimitMEMLOCK=infinity` 允许 mlock 锁定全部模型权重。`TimeoutStartSec=300` 防止 systemd 在长时间模型加载时杀死服务。
...
providers:
  custom:local-llm:                                    # ⚠️ 必须包含 "custom:" 前缀，与 model.provider 匹配
    ...
    # 注意：35xb（APEX I-Balanced）未注册到 Hermes——仅由 llama-server 提供手动切换
...
- 35xb（APEX I-Balanced）**未注册**到 Hermes——仍由 llama-server 提供手动 API 切换，但 Hermes 不会路由到它
...
**状态：** 已缓解——当前服务配置已禁用 prompt cache（`--cache-ram 0`）
...
- prompt cache 已禁用（`--cache-ram 0`）防止无限 cache 积累
...
**警告：** `--cache-ram -1` 仅在单模型轮换模式且可控负载下可安全使用。双模型常驻模式下（`models-max 2`），128 GB 统一内存中两个模型共占用 41–59 GB，一个模型的无限 prompt cache 会在冷加载时挤占另一模型的内存。当前部署使用 `--cache-ram 0`（禁用）以保稳定。
...
*测试环境：{your_machine} · AMD Ryzen AI Max+ 395 · 128 GB · llama.cpp b9315 Vulkan · 2026-06-03 · 更新于 2026-06-08（主力 278，辅助 358，35xb Hermes 未注册；Nginx：server_tokens off, gzip 全局关闭按 location 独立控制, 64m body, 限流 60r/m；Hermes：approvals.gateway_timeout 3600, cache-ram 0, ubatch 256）*
