...
| Prompt | F16 ub=256 | Q8_0 ub=512 |
|--------|-----------|-------------|
| p256K | **1041** | 1352 |

**结论：**

1. **Q8_0 KV UB=1024 是 Q8_0 配置中的最优**——p4K–p128K prefill 最快，gen 和 TTFT 接近最优；但在 p64K+ 仍不如 F16 KV
2. **Q8_0 KV UB=512 是唯一能完成 256K 的 Q8_0 配置**——prefill 199.9 t/s，gen 24.3 t/s；但仍不如 F16 KV UB=256（prefill -16%，gen -14%，总耗时 +30%）
3. **Q8_0 KV UB=4096 全面劣化**——p4K+ prefill 慢 30~40%，gen 慢 7~33%，256K 在 ~160K tokens 处 Vulkan 崩溃
4. **即使最优 Q8_0 配置 (ub=1024) 在 p64K+ 仍不如 F16 ub=256**——prefill -2~13%，gen -7~9%
5. **结论：35B MoE 维持 F16 KV UB=256。** Vulkan FA 反量化开销超过 Q8_0 压缩节省的带宽；MoE 稀疏 KV cache（3B 激活参数）意味着可节省的带宽本就有限。Q8_0 KV 仅在短上下文 prefill（≤32K）有收益；256K 上下文即使 UB=512（唯一可行的 Q8_0 配置）仍比 F16 KV UB=256 慢 30%。
...
Q8_0 KV cache 在 4 种 UB 值（256–4096）下测试，对比 F16 KV UB=256 基线。UB≥2048 在 128K+ 上下文崩溃。
...
| p128K | 49.8 | 115.8 | **185.1** | 115.3 | 116.9 | ❌ 崩溃 |
| p256K | ❌ | 80.8 | **138.6** | 136.7 | ❌ 崩溃 | ❌ 崩溃 |
...
| p128K | **10.0** | 9.5 | 9.3 | 8.9 | 9.0 | ❌ 崩溃 |
| p256K | ❌ | **7.2** | 6.8 | 6.7 | ❌ 崩溃 | ❌ 崩溃 |
...
| p256K | ❌ | 2994.1s | **1891.4s** | 1917.1s | ❌ 崩溃 |
...
5. **UB≥2048 Vulkan 崩溃**——UB=2048 p256K 崩溃，UB=4096 p128K 崩溃；27B Dense VRAM 余量比 35B MoE 更紧张
6. **UB=512 p256K 总执行时间 33 分钟 vs UB=256 的 53 分钟**——每次 256K 请求省 20 分钟

</details>

> **已从活跃名单移除。** Gen 速度仅 7–13 t/s（Q6 ~17 t/s，Q4 ~21 t/s）。Q8_0 KV 可启用 256K 上下文并大幅提升长上下文 prefill，但 gen 仍慢。建议使用 Q6 或 Q4。
