...
| 场景 | 配置 | Gen 速度 | Prefill 速度 | TTFT | MTP 命中率 |
|------|------|----------|-------------|------|-----------|
...
| 256K 长上下文 | `-c 262144 -b 4096 -ub 256 -t 8` | 29.7 t/s | ~248 t/s | ~1010s | 71.1% |

> MoE 架构每个 token 仅激活 35B 中的 3B 参数 → 内存带宽消耗极低 → 生成速度最快。
>
> **KV cache 量化 (Q8_0) 实测无收益：** Prefill 随上下文增长衰减 3%–21%（Vulkan FA 反量化开销主导）；Gen 速度基本持平（±3–5%）；MTP 不受影响。结论：维持默认 F16 KV cache。
