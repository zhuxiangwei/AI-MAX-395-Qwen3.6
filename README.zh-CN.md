...
APEX 量化——针对 MoE 的自适应精度策略。混合精度 per tensor（关键层 Q6_K/Q8_0，中间 expert 层 Q4_K_M）。整体 ~22 GB（按体积介于 Q4~Q5，但质量接近 Q8）。imatrix 多样化校准。**比原版 Q8+MTP 快 48%，体积仅 59%。**

**最优配置：F16 KV cache。** 与原版 35B Q8 规律一致：≤128K → UB=512（prefill +15~23%）；256K → UB=256（prefill 比 UB=512 慢 4%）。
...
> APEX I-Quality 生成速度在所有 prompt 长度下**比原版 Q8 快约 48%**（80 vs 54 t/s）。文件大小 21.9 GB vs 37 GB。
...
> APEX I-Balanced 生成速度**比原版 Q8 快约 40%**。质量领先：KL max 4.53 为所有量化最优。
...
> APEX I-Balanced 智力得分最高（55/80）——量化越轻推理能力保留越好。I-Quality 速度最快但推理错误更多（尤其三段论 0/10）。三者均在三段论上表现不佳（关键词评分可能过严）。358 在说谎者悖论拿到满分，APEX 模型均得 7/10。
...
> 视觉模式下 MTP 命中率（48–57%）明显低于纯文本模式（60–70%），因为视觉 token 更难预测。APEX I-Quality 视觉生成比原版 Q8 快约 39%。三个模型均能准确描述图片内容。
...
> **别名命名规则：** APEX 模型使用 `35q`/`35b` 表示质量/平衡。原版 UD 量化模型使用 3 位数字 = 模型大小 + 量化等级。API 请求的 `model` 字段使用别名或完整文件名均可。
