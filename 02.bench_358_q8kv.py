#!/usr/bin/env python3
"""02.bench_358_q8kv.py — 35B MoE Q8 Q8_0 KV + UB sweep benchmark.

Tests 358 (35B-A3B Q8 MoE) at p128/p4K/p32K/p64K/p128K/p256K
with Q8_0 KV cache across multiple UB values (256/512/1024/2048/4096).

Usage (on inference machine):
    LLM_BASE_DIR=/home/zxw LLM_API_KEY=xxx python3 -u 02.bench_358_q8kv.py
"""
...
# UB values to sweep
UB_VALUES = [256, 512, 1024, 2048, 4096]
...
def main():
    print("=" * 64)
    print("  35B MoE Q8 — Q8_0 KV + UB Sweep Benchmark")
    ...
    try:
        for ub in UB_VALUES:
            csv_file = os.path.join(_BASE_DIR, f"test/bench_358_q8kv_ub{ub}.csv")
            results = []
            ...
            for i, (name, target_tok) in enumerate(TEST_POINTS):
                ...
                if not server.start():
                    ...
                    break  # UB value is likely unstable, skip remaining
                ...
                if "error" in result:
                    ...
                    server.stop()
                    break  # Stop this UB, move to next
                else:
