...
- `auxiliary` — 11 个辅助任务：9 个路由到 35xb（`provider: custom:local-llm`），2 个回退到默认模型 278（`provider: auto`：kanban_decomposer、curator）；`enable_thinking: false` 降低延迟；`timeout: 3600` 与全链路超时对齐
...
*测试环境：{your_machine} · AMD Ryzen AI Max+ 395 · 128 GB · llama.cpp b9315 Vulkan · 2026-06-03 · 更新于 2026-06-06*
