...
> † 当前配置（F16 KV, UB=512, kv-unified=1, cache-ram=32768, reasoning-budget=16384, max_cstate=1, governor=performance）下的初步测量。完整基准测试待补。
>
...
| 按量化等级差异化 ub | 量化越高权重越大，VRAM 余量越小，需更小 ub 保证稳定性；最优 UB 因模型而异（256–1024）。当前部署：活跃模型统一 UB=512（278, 358）；UB=512 在 358 MoE ≤128K 最优；278 同样使用 UB=512 |
| `cache-ram = 32768`（INI 中每模型配置） | prompt cache 已启用，每模型 32 GB。此前服务配置使用 `--cache-ram 0`（禁用），现已迁至 INI 每模型配置。32 GB 为单模型轮换模式下的长上下文 prompt 复用提供充足缓存 |
...
| 35B MoE: UB 约束 | UB=512（当前，≤128K 最优） | UB≥1024 在 p256K 劣化；UB≥2048 Vulkan 崩溃 |
...
| `cache-ram` | `32768`（32 GB 每模型，在 INI 中设置） | prompt cache 在 INI 中每模型配置（此前服务级 `0` 禁用）；`--cache-ram -1` 曾导致无限增长但收益不成比例；单模型轮换缓解争抢（见已知问题） |
...
>
> **部署模式：**
> - **单模型轮换**：models-max 1 → 一次加载一个模型，通过 LRU 按需切换（冷加载 30–60 秒），配合 `--slot-save-path` 保存/恢复 KV checkpoint（INI 中每模型配置）。GTT 120GB + mlock=1 + cache-ram=32768（INI 中每模型配置）。不加 --sleep-idle-seconds。
>
...
> 单模型轮换模式：`--models-max 1`，`--slot-save-path` 和 `--cache-ram` 现在在 preset INI 中每模型配置。模型切换延迟 30–60 秒（冷加载 + slot restore）。`--timeout 3600` 覆盖 278 上 256K prefill 最坏情况（~3022s）并留有余量。GTT 120GB + mlock=1 确保模型权重常驻物理内存。`LimitMEMLOCK=infinity` 允许 mlock 锁定全部模型权重。`TimeoutStartSec=300` 防止 systemd 在长时间模型加载时杀死服务。
...
- [ ] 单模型轮换模式：启动后仅 278 显示 status `loaded`，358 在首次请求时冷加载（<15 秒）
...
**状态：** 已缓解——prompt cache 现在在 INI 中每模型配置（`cache-ram = 32768`），不再使用服务级 `--cache-ram -1`
...
**当前缓解措施：**
- 切换至单模型轮换模式（`--models-max 1`），同一时刻仅一个模型在内存中
- Prompt cache 在 INI 中每模型配置为固定上限（`cache-ram = 32768`，32 GB），防止无限增长
...
*测试环境：{your_machine} · AMD Ryzen AI Max+ 395 · 128 GB · llama.cpp b9592 Vulkan · 2026-06-03 · 更新于 2026-06-12（主力 278，辅助 358，35xb 已删除；INI：kv-unified=1, cache-ram=32768, reasoning-budget=16384, ubatch=512, 采样参数；Hermes：title_generation→278）*
