...
> MTP 接受率：均值 87.1%，中位数 89.2%。短/中上下文生成稳定在 11–13 t/s。
...
| 别名 | 文件 | 架构 | 大小 | 激活参数 | 角色 |
|------|------|------|------|----------|------|
| **278** | `Qwen3.6-27B-UD-Q8_K_XL.gguf` | Dense | ~33 GB | 27B | 主模型（Hermes 默认，支持视觉） |
| **358** | `Qwen3.6-35B-A3B-UD-Q8_K_XL.gguf` | MoE | ~37 GB | 3B | 语音助手（快速响应） |

双模型模式（`--models-max 2`），Router 通过 LRU 自动切换。
...
while true; do
    ...
    cpu=${cpu:+$((cpu / 1000))}
    nvme=${nvme:+$((nvme / 1000))}
    echo "$ts gpu_busy=${busy:-0}% gpu=${gpu:-NA}°C cpu=${cpu:-NA}°C nvme=${nvme:-NA}°C" >> "$LOGFILE"
    sleep "$INTERVAL"
done
```
...
exec "$SERVER" \
  ...
  --models-max 2 \
  --models-preset "$MODELS_DIR/router-preset.ini" \
  --timeout 3600 \
...
BINDIR="/home/zxw/llama.cpp/build/bin"
SERVER="$BINDIR/llama-server"
MODEL="/home/zxw/model-tts/Qwen3-TTS-12Hz-0.6B-CustomVoice/ggml-model.gguf"
...
- [ ] `llama-router.service` 活跃（`--models-max 2`）
